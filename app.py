    ...
    if st.session_state.submitted:
        ...
        if cp == 0:
            ...
            with col2:
                ...
                nationality = st.text_input("الجنسية (بالإنجليزية)", placeholder="e.g. Egyptian",
                                            value=st.session_state.nationality_saved,
                                            key="nationality_input")
                st.session_state.nationality_saved = nationality
                if any('\u0600' <= c <= '\u06ff' for c in (nationality or "")):
                    st.markdown('<div style="color:#D9534F;font-size:.82rem;direction:rtl;">⚠ يرجى كتابة الجنسية باللغة الإنجليزية فقط.</div>', unsafe_allow_html=True)
            with col3:
                ...
                referral = st.text_input("مصدر الإحالة (بالإنجليزية)", placeholder="e.g. Dr. Ahmed Hassan",
                                         value=st.session_state.referral_saved,
                                         key="referral_input")
                st.session_state.referral_saved = referral
                if any('\u0600' <= c <= '\u06ff' for c in (referral or "")):
                    st.markdown('<div style="color:#D9534F;font-size:.82rem;direction:rtl;">⚠ يرجى كتابة مصدر الإحالة باللغة الإنجليزية فقط.</div>', unsafe_allow_html=True)
            ...
            if submit and all_answered and not has_arabic:
                with st.spinner("جاري معالجة إجاباتك..."):
                    ...
                    def strip_arabic(text):
                        """Remove Arabic characters to prevent PDF rendering errors."""
                        return "".join(c for c in (text or "") if not ('\u0600' <= c <= '\u06ff')).strip() or "Not provided"

                    dob_v         = st.session_state.get("dob_saved","") or "Not provided"
                    nationality_v = strip_arabic(st.session_state.get("nationality_saved",""))
                    referral_v    = strip_arabic(st.session_state.get("referral_saved",""))
